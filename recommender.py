...
def _content_based_scores(resources: QuerySet[LearningResource], pref: UserPreference | None) -> dict[int, float]:
    ...
    ids: List[int] = []
    for r in resources:
        tag_names = list(r.tags.values_list("name", flat=True))
    ...
    if profile_tokens:
        ...
        return {rid: float(score) for rid, score in zip(ids, sims)}

    # If no profile, give neutral content scores
...
def recommend_for_user(user_id: int, limit: int = 12) -> QuerySet[LearningResource]:
    """
    Hybrid recommender:
    ...
    try:
        ...
        qs = qs.distinct()
        if not qs.exists():
            return LearningResource.objects.all().order_by("-created_at")[:limit]
        ...
        # Preserve score ordering using CASE expression
        ordering = {rid: idx for idx, rid in enumerate(top_ids)}
        preserved = "CASE " + " ".join(f"WHEN id={rid} THEN {idx}" for rid, idx in ordering.items()) + " END"
        return qs.filter(id__in=top_ids).extra(select={"_order": preserved}).order_by("_order")
    except Exception:
